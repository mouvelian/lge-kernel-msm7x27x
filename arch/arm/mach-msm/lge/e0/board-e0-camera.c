...
static void msm_camera_vreg_config(int vreg_en)
{
	...
	if (vreg_en) {
		...
		rc = bu61800_ldo_enable(NULL,4,vreg_en); 				//IOVDD: 1.8V START
		if (rc < 0) {
			pr_err("%s: aat28xx_ldo_enable(ldo4) failed\n", __func__);
		}									//IOVDD: 1.8V END

		rc = bu61800_ldo_enable(NULL,2,vreg_en);				//AVDD: 2.8V START
		if (rc < 0) {
			pr_err("%s: aat28xx_ldo_enable(ldo2) failed\n", __func__);
		}									//AVDD: 2.8V END
		
		rc = bu61800_ldo_enable(NULL,3,vreg_en);				//DVDD: 1.2V START
		if (rc < 0) {
...
static struct platform_device *e0_camera_devices[] __initdata = {
...
void __init lge_add_camera_devices(void)
{
	...
	platform_add_devices(e0_camera_devices,
		ARRAY_SIZE(e0_camera_devices));
}
