...
void __init msm7x27a_e0_init_i2c_subpm(int bus_num)
{
	subpm_i2c_device.id = bus_num;

	lge_init_gpio_i2c_pin(&subpm_i2c_pdata, subpm_i2c_pin,
		&subpm_i2c_bdinfo);
	i2c_register_board_info(bus_num, &subpm_i2c_bdinfo, 1);
...
{
	lge_add_gpio_i2c_device(msm7x27a_e0_init_i2c_subpm);
}
