...
static int gpio_sdc1_hw_det = GPIO_SD_DETECT_N;
...
{
	int rc = 0;
/*[E0][Kernel]Integrating MMC voltage regulator [START]*/
...
void __init msm7627a_init_mmc(void)
{
	 ...
	 * L10 : emmc -> lcd, S3 : msme1 + emmc
	 */
	/* eMMC slot */
#ifdef CONFIG_MMC_MSM_SDC3_SUPPORT
	if (mmc_regulator_init(3, "msme1", 2850000))//S3
                return;
