...
static int e0_gpio_earsense_work_func(int *value)
{
	int state;
	int gpio_value;
	...
	if (gpio_value == EAR_EJECT) {
		...
		*value = 0;
		...
		gpio_set_value(GPIO_MIC_MODE, 1); //Improvement of Headset booting noise.
		msleep(100);
...
static char *e0_gpio_earsense_print_name(int state)
{
	if (state)
		return "Headset";
	else
...
static char *e0_gpio_earsense_print_state(int state)
{
...
static int e0_gpio_earsense_sysfs_store(const char *buf, size_t size)
{
	int state;
...
static unsigned e0_earsense_gpios[] = {
	GPIO_EAR_SENSE,
};

/* especially to address gpio key */
static unsigned e0_hook_key_gpios[] = {
	GPIO_BUTTON_DETECT,
};

static int e0_gpio_hook_key_work_func(int *value)
{
	int gpio_value;
...
static struct lge_gpio_switch_platform_data e0_earsense_data = {
	...
	.name = "h2w",
	/* LGE_UPDATE_E							*/
	.gpios = e0_earsense_gpios,
	.num_gpios = ARRAY_SIZE(e0_earsense_gpios),
	.irqflags = IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
	.wakeup_flag = 1,
	.work_func = e0_gpio_earsense_work_func,
	.print_name = e0_gpio_earsense_print_name,
	.print_state = e0_gpio_earsense_print_state,
	.sysfs_store = e0_gpio_earsense_sysfs_store,

	/* especially to address gpio key */
	.key_gpios = e0_hook_key_gpios,
	.num_key_gpios = ARRAY_SIZE(e0_hook_key_gpios),
	.key_work_func = e0_gpio_hook_key_work_func,
};
...
static struct platform_device *e0_sound_devices[] __initdata = {
	&msm_device_snd,
	&msm_device_adspdec,
	&e0_earsense_device,
//	&asoc_msm_pcm,
...
void __init lge_add_sound_devices(void)
{
	...
	platform_add_devices(e0_sound_devices, ARRAY_SIZE(e0_sound_devices));
}
